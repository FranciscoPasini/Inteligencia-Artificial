...
public class Pathfinding
{
    ...
    public static List<T> ThetaStar<T>(T start, Func<T, bool> satisfies,
        Func<T, List<T>> getNeighbors, Func<T, T, float> getCost, Func<T, float> getHeuristic, Func<T, T, bool> lineOfSight) where T : class
    {
        PriorityQueue<T> frontier = new();
        frontier.Enqueue(start, 0);
        Dictionary<T, T> cameFrom = new();
